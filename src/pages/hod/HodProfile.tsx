...
import { Mail, Lock, Edit2, Save, X, AlertTriangle, CheckCircle2, Building2 } from "lucide-react";
...
async function fetchHodProfile(): Promise<HodProfileData | null> {
  try {
...
export function HodProfile() {
  ...
  const [form, setForm] = useState<HodProfileData | null>(null);
  ...
  const onPhotoSelect = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/hod/profile/photo`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token()}` },
        body,
      });
  ...
  const saveProfile = async () => {
    ...
    setSaveError(null);
    setSaveSuccess(null);
    const res = await fetch(`${API_BASE}/api/hod/profile/update`, {
      method: "PUT",
  ...
  return (
    <div>
      <PageHeader title="My Profile" subtitle="Head of Department account settings" />
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full mt-1 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900"
                    />
                  </div>
                  <div className="flex gap-2">
                    <Button onClick={saveProfile} variant="primary"><Save className="w-4 h-4" /> Save</Button>
                    <Button onClick={() => { setEditing(false); setForm(profile); }} variant="secondary"><X className="w-4 h-4" /> Cancel</Button>
                  </div>
                </div>
              ) : (
